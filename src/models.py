...
class MllmBrainToTextV0(nn.Module):
    ...
    def __init__(
        ...
        model = model.float()
        model.load_state_dict(torch.load('trained_models/DeconvBipartiteTransformerConv.pt', weights_only=True))
        ...
        #self.frmi_encoder = torch.load('benchmark_transformers_src/trained_models/DeconvBipartiteTransformerConv.pt', map_location=torch.device(self.device)).encoder.to(self.device)
